module Concerns::DefaultPost
  ...
  NAKASHII_GOROKU_FOR_TITLES = %w(
    俺が中島だ
    凸した結果wwwww
  )
  NAKASHII_GOROKU_FOR_CONTENTS = %w(
  ...
  included do
    def fill_in_blank_params
      if params[:title].blank?
        params[:title] = NAKASHII_GOROKU_FOR_TITLES[rand(NAKASHII_GOROKU_FOR_TITLE.size)]
      end
      if params[:name].blank?
        params[:name] = "nakashii_"
      end
      if params[:content].blank?
        params[:content] = NAKASHII_GOROKU_FOR_CONTENTS[rand(NAKASHII_GOROKU_FOR_CONTENTS.size)]
      end
    end
  end
end